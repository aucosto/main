...
- **SQLite + `@prisma/adapter-better-sqlite3`** for local dev. Slated to move to Supabase Postgres later.
...
2. Prisma 7 does **not** accept `url = env("...")` in `schema.prisma`. The URL lives in `prisma.config.ts`, and the runtime client requires a driver adapter (`new PrismaClient({ adapter: new PrismaBetterSqlite3(...) })`).
...
To re-run a single Prisma migration locally: delete `dev.db` and `prisma/migrations/*` (or use `prisma migrate reset`), then `db:migrate` + `db:seed`.
